...
void FGaussianBlurFilter::BuildShaders()
{
	mHorizontalComputeShader = FDXUtility::CompileShader(
		std::wstring(PROJECT_DIR_W) + L"\\Shaders\\HorizontalGaussianBlurComputeShader.hlsl",
		nullptr,
		"MainCS",
		"cs_5_1"
	);
	mVerticalComputeShader = FDXUtility::CompileShader(
		std::wstring(PROJECT_DIR_W) + L"\\Shaders\\VerticalGaussianBlurComputeShader.hlsl",
		nullptr,
		"MainCS",
		"cs_5_1"
	);
